...
function getCurrentTime() {
    const date = new Date(2022, 3, 14, 0, 0, 0); // Año, mes (0-indexed), día, horas, minutos, segundos
    ...
    let monthDiff = cMonth - 3; // Restamos 3 porque la fecha base es abril (mes 3).
    let yearDiff = cYear - 2022; // Restamos el año base.

    // Si el día actual es anterior al día 14 del mes, restamos un mes.
    if (cDate.getDate() < 14) {
        monthDiff--;
    }

    // Si el mes actual es anterior al mes base (abril), restamos un año.
    if (cMonth < 3) {
        yearDiff--;
        monthDiff += 12; // Sumamos 12 meses para convertir a meses positivos.
    }

    // Si el cálculo de los meses es negativo, ajustamos los años.
    if (monthDiff < 0) {
        monthDiff += 12;
        yearDiff--;
    }

    // Si quedan menos de 365 días, mostramos solo 1 año.
    if (dayM < 365) {
        yearDiff = 1;
    }

    let countYear = yearDiff < 10 ? "0" + yearDiff : yearDiff;
    let countMonth = monthDiff < 10 ? "0" + monthDiff : monthDiff;
...
function spinWheel() {
    ...
    setTimeout(function () {
        ...
        if (wItem.length < 6) wItem = ':' + wItem;
        infoSpin.innerText = `Felicidades mi princesa, ganaste ${wItem}!`
        // spinBtn.addEventListener('click',spinWheel)
    }, 5000)
}
